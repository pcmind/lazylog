            ...
            let batch_matches = tokio::task::spawn_blocking(move || {
                ...
                if source_indices.is_none() {
                    ...
                    if first_line < current_offsets.len() {
                        ...
                        let chunk_end = current_offsets[std::cmp::min(last_line + 1, current_offsets.len() - 1)];
                        let chunk_size = chunk_end - chunk_start;

                        if chunk_size > 0 {
                            buf.resize(chunk_size as usize, 0);
                            if std_file_clone.seek(std::io::SeekFrom::Start(chunk_start)).is_ok() && 
                               std_file_clone.read_exact(&mut buf).is_ok() {
                                
                                for i in local_last..batch_end {
                                    if processed > 0 && processed % 1000 == 0 {
                                        if task_gen_clone.load(std::sync::atomic::Ordering::Relaxed) != expected_gen_clone { break; }
                                    }
                                    if i + 1 >= current_offsets.len() { break; }
                                    ...
                                    if line_end <= buf.len() {
                                        ...
                                        if is_negated { matched = !matched; }
                                        if matched { new_matches.push(i); }
                                    }
                                    processed += 1;
                                }
                            }
                    ...
                    for i in local_last..batch_end {
                        if processed > 0 && processed % 1000 == 0 {
                            if task_gen_clone.load(std::sync::atomic::Ordering::Relaxed) != expected_gen_clone { 
                                break; 
                            }
                        }
