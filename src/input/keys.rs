...
impl KeyRegistry {
    pub fn default_bindings() -> Self {
        ...
        let push1 = |b: &mut Vec<KeyBinding>,
                     code,
                     modifiers,
                     action,
                     label,
        ...
        let push2 =
            |b: &mut Vec<KeyBinding>, c1, c2, action, label, description, context, show_in_bar| {
                b.push(KeyBinding {
                    sequence: vec![
                        KeyCombo::new(KeyCode::Char(c1), KeyModifiers::empty()),
                        KeyCombo::new(KeyCode::Char(c2), KeyModifiers::empty()),
                    ],
                    action,
                    label,
                    description,
        ...
        push1(
            ...
            KeyCode::Char('p'),
            KeyModifiers::empty(),
            ActionId::TogglePinFilter,
            "Pin",
            "Toggle keeping last filter visible",
            BindingContext::Always,
            true,
        );
        push1(
        ...
        push1(
            ...
            KeyCode::Char(' '),
            KeyModifiers::empty(),
            ActionId::ShowLineDetail,
            "Detail",
            "Show full line in a popup",
            BindingContext::Always,
            true,
        );
        ...
        push2(
            ...
            'e',
            'n',
            ActionId::ToggleNegate,
            "Negate",
            "Toggle negate filter",
            BindingContext::FilterPane,
            true,
        );
        push2(
        ...
        push2(
            ...
            'e',
            'b',
            ActionId::ToggleInterleave,
            "Bookmarks",
            "Toggle viewing bookmarked lines",
            BindingContext::FilterPane,
            true,
        );
    ...
    pub fn visible_bindings(
        ...
        for b in &self.bindings {
            if !b.sequence.starts_with(pending) {
                continue;
            }

            // When no pending sequence, only show important bindings
            if pending.is_empty() && !b.show_in_bar {
                continue;
            }
            ...
            if !search_active
                && matches!(
                    b.action,
                    ActionId::NextSearchResult | ActionId::PrevSearchResult
                )
            {
                continue;
            }
            ...
            let context_match = match (b.context, context) {
                (BindingContext::Always, BindingContextWrapper::VisualMode) => {
                    matches!(
                        b.action,
                        ActionId::ScrollDown | ActionId::ScrollUp | ActionId::Quit
                    )
                }
